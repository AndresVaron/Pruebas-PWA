...
(function () {
  ...
  document.getElementById("butAddCity").addEventListener("click", function () {
    ...
    app.saveSelected({ key: key, label: label });
  });

  document
    .getElementById("butAddCancel")
    .addEventListener("click", function (ë) {
  ...
  async function load() {
    ...
    var store = tx.objectStore("stations");
    let stations = await store.getAll();
    if (stations.length == 0) {
      ...
      await store.add(item, "metros/1/bastille/A");
    } else {
      app.selectedTimetables = stations.map((station) => {
        return { key: station.key, label: station.label };
      });
